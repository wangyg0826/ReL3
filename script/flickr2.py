...
def ffmpeg(ffmpeg_path, input_path, video_output_path=None,
    ...
    last_err = None
    input_path = 'https://www.flickr.com/video_download.gne?id={}'.format(input_path[:-4])
    for attempt in range(num_retries):
        try:
            args = [ffmpeg_path]
            # Add input arguments
            args += input_args + ['-i', input_path , "-stimeout", "20000"]
...
def download_flickr_video(url_queue, cv_main_to_worker, cv_worker_to_main, data_dir, ffmpeg_path, ffprobe_path,
    ...
    while True:
        while True:
            try:
                if stop:
                    LOGGER.info('Worker process {} stopped'.format(os.getpid()))
                    return
...
def ffprobe(ffprobe_path, filepath):
    ...
    """
    cmd_options = '-v quiet -print_format json -show_format -show_streams'.split()
    cmd = [ffprobe_path]
